class Game {
    ...
    loadHighScore() {
        const savedHighScore = localStorage.getItem(this.highScoreKey);
    ...
    update() {
        if (this.player.isAlive() && this.started) {
            ...
            for (let b of this.bg) {
                b.update(Math.floor(this.speed * .7));
            }
            ...
            let highScore = localStorage.getItem(this.highScoreKey);
            if (!highScore) {
                highScore = 0;
            } else {
            ...
            let btnEasy = createButton('Facile');
            btnEasy.position(windowWidth / 2 - 200, windowHeight / 2 - 200);
            btnEasy.mousePressed(() => setDifficulty('easy'));

            let btnMoyen = createButton('Moyen');
            btnMoyen.position(windowWidth / 2 - 100, windowHeight / 2 - 200);
            btnMoyen.mousePressed(() => setDifficulty('moyen'));

            let btnHard = createButton('Difficile');
            btnHard.position(windowWidth / 2 , windowHeight / 2 - 200);
            btnHard.mousePressed(() => setDifficulty('hard'));

            let btnImp = createButton('Impossible');
            btnImp.position(windowWidth / 2 + 100 , windowHeight / 2 - 200);
            btnImp.mousePressed(() => setDifficulty('impossible'));
            ...
            textSize(32);
            fill(32, 33, 36);
            text("Appuyez sur espace pour débuter la partie", window.innerWidth / 2 - 100, this.textY);

            this.textY += this.textSpeed;
            if (this.textY > window.innerHeight / 2 + 120 || this.textY < window.innerHeight / 2 + 40) {
                this.textSpeed *= -1;
            }
        }
    ...
    load_player_assets() {
        ...
        this.player.imgs[0] = this.player.img_running_1; this.player.imgs[1] = this.player.img_running_2;
        //  this.player.imgs[2] = this.player.img_running_3;
    ...
    load_ground_assets() {
        this.ground.img = this.sprite.get(2, 53, 1173, 200);
        this.ground.imgGameNotStarted = this.sprite.get(1179, 53, 49, 200);
    }

    spawn_enemy() {
        if (Math.floor(random(10)) == 0) {
            if (this.score > 300) { // value important
    ...
    despawn_bg() {
        this.bg = this.bg.filter(b => b.x + b.w >= 0);
    }
